...
Response validation is only applied to the responses generated by your application before the Firetail middleware - the Firetail middleware does not validate its own responses against the openapi spec, as this could lead to behaviours where your API will never passes its own validation, and so never return. This discrepancy is an issue we are currently exploring a number of possible solutions for.
